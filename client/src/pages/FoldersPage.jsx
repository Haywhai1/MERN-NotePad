...
import { FolderIcon, PlusIcon, FolderPlusIcon, StarIcon, TrashIcon } from "@heroicons/react/24/solid";
...
export default function FoldersPage() {
  ...
  return (
    <div className="bg-black text-white min-h-screen w-full h-full flex flex-col relative px-4">
      <h1 className="text-4xl font-bold mb-4 text-orange-500 p-5">Folders</h1>

      <SearchInput value={searchTerm} onChange={setSearchTerm} placeholder="Search folders..." />

      <div className={`flex-1 overflow-y-auto  ${showAddFolder ? "blur-sm" : ""}`} style={{ paddingBottom: "100px" }}>
        <div className="bg-neutral-800 rounded-lg p-2">
          ...
          {filteredFolders.map((folder, index) => (
            <div
              key={folder._id}
              onClick={() =>
                navigate(`/folder/${folder._id}`, { state: { folderName: folder.name } })
              }
              className="w-full p-4 cursor-pointer hover:bg-neutral-700 transition-all flex items-center gap-4"
            >
              <FolderIcon className="w-7 h-7 text-orange-400 flex-shrink-0" />
              <div className="flex-1">
                <p className="text-xl font-semibold">{highlightText(folder.name, searchTerm)}</p>
                <p className="text-sm text-neutral-400">{folder.notes?.length || 0} notes</p>
                {index !== filteredFolders.length - 1 && <div className="border-b border-white/20 mt-4"></div>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-neutral-900 border-t border-white/20 p-3 flex justify-around z-10">
        <button onClick={handleAddNote} className="bg-orange-500 hover:bg-orange-400 p-4 rounded-full shadow-lg transition">
          <PlusIcon className="w-6 h-6 text-white" />
        </button>
        <button onClick={handleAddFolderClick} className="bg-orange-500 hover:bg-orange-400 p-4 rounded-full shadow-lg transition">
          <FolderPlusIcon className="w-6 h-6 text-white" />
        </button>
        <button onClick={handleViewFavorites} className="bg-orange-500 hover:bg-orange-400 p-4 rounded-full shadow-lg transition">
          <StarIcon className="w-6 h-6 text-white" />
        </button>
        <button onClick={() => navigate("/recently-deleted")} className="bg-orange-500 hover:bg-orange-400 p-4 rounded-full shadow-lg transition">
        ...
        <div className="absolute inset-0 bg-black/50 backdrop-blur-sm z-10 flex justify-center items-center">
          <div className="bg-neutral-800 p-6 rounded-lg w-80 flex flex-col space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowAddFolder(false)} className="px-4 py-2 bg-gray-600 hover:bg-gray-500 rounded transition">
                Cancel
              </button>
              <button
